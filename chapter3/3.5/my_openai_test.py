...
def evaluate_qa_pair(idx, qa_pairs):
    ...
    위의 질문에 대한 두 답변을 비교 평가해주세요.
    다음의 기준에 따라 1-10점 사이의 점수를 매겨주세요:
...
def main():
    ...
    qa_pairs = pd.DataFrame(data, columns=["question", "answer", "generation"]) # data frame 생성 (틀짜기)
    qa_pairs = data # 틀에 실제 데이터 저장
    
    # qa_pairs에 인덱스 추가
    indexed_qa_pairs = list(range(len(qa_pairs)))
    # partial 함수를 사용해 qa_pairs를 evaluate_qa_pair에 전달
    # partial 함수의 역할?
    evaluate_func = partial(evaluate_qa_pair, qa_pairs=qa_pairs)
    # pqdm을 사용해 병렬 처리
    # 처리할 data, data를 적용할 함수, 몇 개의 병렬?
